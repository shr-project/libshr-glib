...
/**
 * phoneui_settings_override_properties:
 * @klass: The class structure for a #GObject<!-- -->-derived class.
 * @property_id_begin: The property id to assign to the first overridden property.
 *
 * Overrides all #GObject properties in the #PhoneuiSettings interface for a concrete class.
 * The properties are overridden in the order they are defined.
 *
 * Returns: The last property id.
 */
guint
phoneui_settings_override_properties (GObjectClass *klass, guint property_id_begin)
{
...
phoneui_settings_default_init (PhoneuiSettingsIface *iface)
{
   ...
   * @object: A #PhoneuiSettings.
   * @invocation: A #GDBusMethodInvocation.
   * @arg_message_receipt: Argument passed by remote caller.
   *
 ...
 * @proxy: A #PhoneuiSettingsProxy.
 * @arg_message_receipt: Argument to pass with the method invocation.
 * @cancellable: (allow-none): A #GCancellable or %NULL.
 * @callback: A #GAsyncReadyCallback to call when the request is satisfied or %NULL.
 * @user_data: User data to pass to @callback.
 *
...
struct _PhoneuiSettingsProxyPrivate
{
  GData *qdata;
};

static void phoneui_settings_proxy_iface_init (PhoneuiSettingsIface *iface);
...
static void
phoneui_settings_proxy_finalize (GObject *object)
{
  PhoneuiSettingsProxy *proxy = PHONEUI_SETTINGS_PROXY (object);
  g_datalist_clear (&proxy->priv->qdata);
  G_OBJECT_CLASS (phoneui_settings_proxy_parent_class)->finalize (object);
}
...
static void
phoneui_settings_proxy_g_properties_changed (GDBusProxy *_proxy,
  GVariant *changed_properties,
  const gchar *const *invalidated_properties)
{
  PhoneuiSettingsProxy *proxy = PHONEUI_SETTINGS_PROXY (_proxy);
  guint n;
  const gchar *key;
  GVariantIter *iter;
...
{
  proxy->priv = G_TYPE_INSTANCE_GET_PRIVATE (proxy, TYPE_PHONEUI_SETTINGS_PROXY, PhoneuiSettingsProxyPrivate);
...
phoneui_settings_proxy_class_init (PhoneuiSettingsProxyClass *klass)
{
  ...
  g_type_class_add_private (klass, sizeof (PhoneuiSettingsProxyPrivate));

  gobject_class = G_OBJECT_CLASS (klass);
  gobject_class->finalize     = phoneui_settings_proxy_finalize;
  ...
  gpointer user_data)
{
  ...
  n = 0;
  g_value_init (&paramv[n], TYPE_PHONEUI_SETTINGS);
...
static void phoneui_settings_skeleton_iface_init (PhoneuiSettingsIface *iface);
