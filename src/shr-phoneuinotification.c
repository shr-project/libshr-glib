...
/**
 * phoneui_notification_override_properties:
 * @klass: The class structure for a #GObject<!-- -->-derived class.
 * @property_id_begin: The property id to assign to the first overridden property.
 *
 * Overrides all #GObject properties in the #PhoneuiNotification interface for a concrete class.
 * The properties are overridden in the order they are defined.
 *
 * Returns: The last property id.
 */
guint
phoneui_notification_override_properties (GObjectClass *klass, guint property_id_begin)
{
...
phoneui_notification_default_init (PhoneuiNotificationIface *iface)
{
   ...
   * @object: A #PhoneuiNotification.
   * @invocation: A #GDBusMethodInvocation.
   * @arg_dialog: Argument passed by remote caller.
   *
 ...
 * @proxy: A #PhoneuiNotificationProxy.
 * @arg_dialog: Argument to pass with the method invocation.
 * @cancellable: (allow-none): A #GCancellable or %NULL.
 * @callback: A #GAsyncReadyCallback to call when the request is satisfied or %NULL.
 * @user_data: User data to pass to @callback.
 *
...
struct _PhoneuiNotificationProxyPrivate
{
  GData *qdata;
};

static void phoneui_notification_proxy_iface_init (PhoneuiNotificationIface *iface);
...
static void
phoneui_notification_proxy_finalize (GObject *object)
{
  PhoneuiNotificationProxy *proxy = PHONEUI_NOTIFICATION_PROXY (object);
  g_datalist_clear (&proxy->priv->qdata);
  G_OBJECT_CLASS (phoneui_notification_proxy_parent_class)->finalize (object);
}
...
static void
phoneui_notification_proxy_g_properties_changed (GDBusProxy *_proxy,
  GVariant *changed_properties,
  const gchar *const *invalidated_properties)
{
  PhoneuiNotificationProxy *proxy = PHONEUI_NOTIFICATION_PROXY (_proxy);
  guint n;
  const gchar *key;
  GVariantIter *iter;
...
{
  proxy->priv = G_TYPE_INSTANCE_GET_PRIVATE (proxy, TYPE_PHONEUI_NOTIFICATION_PROXY, PhoneuiNotificationProxyPrivate);
...
phoneui_notification_proxy_class_init (PhoneuiNotificationProxyClass *klass)
{
  ...
  g_type_class_add_private (klass, sizeof (PhoneuiNotificationProxyPrivate));

  gobject_class = G_OBJECT_CLASS (klass);
  gobject_class->finalize     = phoneui_notification_proxy_finalize;
  ...
  gpointer user_data)
{
  ...
  n = 0;
  g_value_init (&paramv[n], TYPE_PHONEUI_NOTIFICATION);
...
static void phoneui_notification_skeleton_iface_init (PhoneuiNotificationIface *iface);
