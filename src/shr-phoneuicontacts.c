...
/**
 * phoneui_contacts_override_properties:
 * @klass: The class structure for a #GObject<!-- -->-derived class.
 * @property_id_begin: The property id to assign to the first overridden property.
 *
 * Overrides all #GObject properties in the #PhoneuiContacts interface for a concrete class.
 * The properties are overridden in the order they are defined.
 *
 * Returns: The last property id.
 */
guint
phoneui_contacts_override_properties (GObjectClass *klass, guint property_id_begin)
{
...
struct _PhoneuiContactsProxyPrivate
{
  GData *qdata;
};

static void phoneui_contacts_proxy_iface_init (PhoneuiContactsIface *iface);
...
static void
phoneui_contacts_proxy_finalize (GObject *object)
{
  PhoneuiContactsProxy *proxy = PHONEUI_CONTACTS_PROXY (object);
  g_datalist_clear (&proxy->priv->qdata);
  G_OBJECT_CLASS (phoneui_contacts_proxy_parent_class)->finalize (object);
}
...
static void
phoneui_contacts_proxy_g_properties_changed (GDBusProxy *_proxy,
  GVariant *changed_properties,
  const gchar *const *invalidated_properties)
{
  PhoneuiContactsProxy *proxy = PHONEUI_CONTACTS_PROXY (_proxy);
  guint n;
  const gchar *key;
  GVariantIter *iter;
...
{
  proxy->priv = G_TYPE_INSTANCE_GET_PRIVATE (proxy, TYPE_PHONEUI_CONTACTS_PROXY, PhoneuiContactsProxyPrivate);
...
phoneui_contacts_proxy_class_init (PhoneuiContactsProxyClass *klass)
{
  ...
  g_type_class_add_private (klass, sizeof (PhoneuiContactsProxyPrivate));

  gobject_class = G_OBJECT_CLASS (klass);
  gobject_class->finalize     = phoneui_contacts_proxy_finalize;
  ...
  gpointer user_data)
{
  ...
  n = 0;
  g_value_init (&paramv[n], TYPE_PHONEUI_CONTACTS);
...
static void phoneui_contacts_skeleton_iface_init (PhoneuiContactsIface *iface);
