...
/**
 * phonefso_usage_override_properties:
 * @klass: The class structure for a #GObject<!-- -->-derived class.
 * @property_id_begin: The property id to assign to the first overridden property.
 *
 * Overrides all #GObject properties in the #PhonefsoUsage interface for a concrete class.
 * The properties are overridden in the order they are defined.
 *
 * Returns: The last property id.
 */
guint
phonefso_usage_override_properties (GObjectClass *klass, guint property_id_begin)
{
...
phonefso_usage_default_init (PhonefsoUsageIface *iface)
{
   ...
   * @object: A #PhonefsoUsage.
   * @invocation: A #GDBusMethodInvocation.
   * @arg_apn: Argument passed by remote caller.
 ...
 * @proxy: A #PhonefsoUsageProxy.
 * @arg_apn: Argument to pass with the method invocation.
...
struct _PhonefsoUsageProxyPrivate
{
  GData *qdata;
};

static void phonefso_usage_proxy_iface_init (PhonefsoUsageIface *iface);
...
static void
phonefso_usage_proxy_finalize (GObject *object)
{
  PhonefsoUsageProxy *proxy = PHONEFSO_USAGE_PROXY (object);
  g_datalist_clear (&proxy->priv->qdata);
  G_OBJECT_CLASS (phonefso_usage_proxy_parent_class)->finalize (object);
}
...
static void
phonefso_usage_proxy_g_properties_changed (GDBusProxy *_proxy,
  GVariant *changed_properties,
  const gchar *const *invalidated_properties)
{
  PhonefsoUsageProxy *proxy = PHONEFSO_USAGE_PROXY (_proxy);
  guint n;
  const gchar *key;
  GVariantIter *iter;
...
{
  proxy->priv = G_TYPE_INSTANCE_GET_PRIVATE (proxy, TYPE_PHONEFSO_USAGE_PROXY, PhonefsoUsageProxyPrivate);
...
phonefso_usage_proxy_class_init (PhonefsoUsageProxyClass *klass)
{
  ...
  g_type_class_add_private (klass, sizeof (PhonefsoUsageProxyPrivate));

  gobject_class = G_OBJECT_CLASS (klass);
  gobject_class->finalize     = phonefso_usage_proxy_finalize;
  ...
  gpointer user_data)
{
  ...
  n = 0;
  g_value_init (&paramv[n], TYPE_PHONEFSO_USAGE);
...
static void phonefso_usage_skeleton_iface_init (PhonefsoUsageIface *iface);
