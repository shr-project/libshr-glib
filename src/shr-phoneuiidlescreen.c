...
/**
 * phoneui_idle_screen_override_properties:
 * @klass: The class structure for a #GObject<!-- -->-derived class.
 * @property_id_begin: The property id to assign to the first overridden property.
 *
 * Overrides all #GObject properties in the #PhoneuiIdleScreen interface for a concrete class.
 * The properties are overridden in the order they are defined.
 *
 * Returns: The last property id.
 */
guint
phoneui_idle_screen_override_properties (GObjectClass *klass, guint property_id_begin)
{
...
struct _PhoneuiIdleScreenProxyPrivate
{
  GData *qdata;
};

static void phoneui_idle_screen_proxy_iface_init (PhoneuiIdleScreenIface *iface);
...
static void
phoneui_idle_screen_proxy_finalize (GObject *object)
{
  PhoneuiIdleScreenProxy *proxy = PHONEUI_IDLE_SCREEN_PROXY (object);
  g_datalist_clear (&proxy->priv->qdata);
  G_OBJECT_CLASS (phoneui_idle_screen_proxy_parent_class)->finalize (object);
}
...
static void
phoneui_idle_screen_proxy_g_properties_changed (GDBusProxy *_proxy,
  GVariant *changed_properties,
  const gchar *const *invalidated_properties)
{
  PhoneuiIdleScreenProxy *proxy = PHONEUI_IDLE_SCREEN_PROXY (_proxy);
  guint n;
  const gchar *key;
  GVariantIter *iter;
...
{
  proxy->priv = G_TYPE_INSTANCE_GET_PRIVATE (proxy, TYPE_PHONEUI_IDLE_SCREEN_PROXY, PhoneuiIdleScreenProxyPrivate);
...
phoneui_idle_screen_proxy_class_init (PhoneuiIdleScreenProxyClass *klass)
{
  ...
  g_type_class_add_private (klass, sizeof (PhoneuiIdleScreenProxyPrivate));

  gobject_class = G_OBJECT_CLASS (klass);
  gobject_class->finalize     = phoneui_idle_screen_proxy_finalize;
  ...
  gpointer user_data)
{
  ...
  n = 0;
  g_value_init (&paramv[n], TYPE_PHONEUI_IDLE_SCREEN);
...
static void phoneui_idle_screen_skeleton_iface_init (PhoneuiIdleScreenIface *iface);
